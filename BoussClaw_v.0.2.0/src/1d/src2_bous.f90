    ...
    do i=1,mx
        ...
        elseif (maxval(q(1,i-2:i+2,j-2:j+2)) < 1.d-1) then
            ...
            if (i == 1) then
                ...
                psi(k)=(B_param+.5d0)*hh(i,j)*((s1(i+2,j)-2.d0*s1(i+1,j)+s1(i,j))/dx**2. &
                    +(s2(i+1,j+1)-s2(i+1,j-1)-s2(i,j+1)+s2(i,j-1))/2.d0/dx/dy) &
                    -B_param*g*HH(i,j)*((detax(i+2,j)-2.d0*detax(i+1,j)+detax(i,j))/dx**2 &
            ...
            elseif (i==mx) then
                ...
                psi(k)=(B_param+.5d0)*hh(i,j)*((s1(i,j)-2.d0*s1(i-1,j)+s1(i-2,j))/dx**2. &
                    +(s2(i,j+1)-s2(i,j-1)-s2(i-1,j+1)+s2(i-1,j-1))/2.d0/dx/dy) &
                     -B_param*g*HH(i,j)*( &
                ...
                psi(k)=(B_param+.5d0)*hh(i,j)*((s1(i+1,j)-2.d0*s1(i,j)+s1(i-1,j))/dx**2. &
                  +(s2(i+1,j+1)-s2(i+1,j-1)-s2(i-1,j+1)+s2(i-1,j-1))/4.d0/dx/dy) &
                  -B_param*g*HH(i,j)*( &
