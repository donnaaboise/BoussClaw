...
subroutine b4step1(mbc,mx,meqn,q,xlower,dx,t,dt,maux,aux)
    ...
    use topo_module, only: xlowdtopo,xhidtopo

    use amr_module, only: xlowdomain => xlower
    use amr_module, only: xhidomain => xupper
    use amr_module, only: xperdom,NEEDS_TO_BE_SET
    ...
    ! Local storage
    integer :: index,i,k,dummy
    real(kind=8) :: h,u,xp,runup
...
!!    end forall
    ...
    do i=1,mx
        ...
        if (q(1,i) > dry_tolerance .and. aux(1,i) >= 0.d0) then
            runup = min(runup,xp)
        endif
    enddo
            
    open(unit=747,file='runup.txt',status='unknown')
    write(747,*) t,runup
